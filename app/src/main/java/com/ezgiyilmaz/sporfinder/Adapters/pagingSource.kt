...
class PagingSource(
    ...
    override suspend fun load(params: LoadParams<Query>): LoadResult<Query, Any> {
        return try {
            println("PagingSource: load fonksiyonu çağrıldı")
            ...
            val player = snapshot.documents.map { document ->
                println("İşlenen belge ID'si: ${document.id}")
                ...
                if (whichOne == "oyuncuBul") {
                    GetPlayerModel(
                        ...
                        println("GetPlayerModel oluşturuldu: $it")
                    } 
                } else {
                    GetRivalModel(
                        id = document.id,
                        ...
                        println("GetRivalModel oluşturuldu: $it")
                    }
                }
            }
